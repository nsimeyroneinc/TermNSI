...
<table  style="table-layout: fixed;background-color:#87A96B; border:solid;color:black;width:100%;">
        <tr >
            ...
            <th  width="75%"; style="text-align:center;border:none;font-size:20pt;">Cours : Calculabilité et Décidabilité</th>
        </tr>
</table>
...
- un paramètre ```f``` (le code-source du programme)
- un paramètre  ```n```, qui serait le paramètre d'entrée de ```f```.
...
![image](data/arret.png){: .center width="40%"}


**Exemple** : 

- ```arret(f, 5)``` renverrait ```True```.
- ```arret(f, 15)``` renverrait ```False```. 
...
> On **_suppose qu'il est possible d'écrire_** une telle fonction
...
### Amusons-nous avec ce programme ```arret```.
...
- si le calcul de `f(x)` s'arrète alors la fonction `etrange` va rentrer dans une boucle infinie et donc elle ne s'arrète jamais.
- si le calcul de ```f(x)``` alors la fonction `etrange` ne fait rien et s'arrête immédiatement.

> **Conclusion :**  `etrange(f,x)` s'arrête unniquement si `f(x)` ne s'arrête pas
...
> `paradox(f)` s'arrête uniquement si `f(f)` ne s'arrête jamais.
...
![image](data/paradox.png){: .center width="25%"}
...
![image](data/turing16.jpg){: .center width="25%"}
...
![image](data/turing.png){: .center width="40%"}
...
??? video "Video"
    <iframe width="1175" height="661" src="https://www.youtube.com/embed/13O1qhX4Bqo" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
